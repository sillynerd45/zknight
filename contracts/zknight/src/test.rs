...
use soroban_sdk::{testutils::Address as _, Address, Bytes, BytesN, Env, Vec, vec};
...
fn test_get_open_games() {
    ...
    let game2 = client.create_game(&player1);
    ...
    let puzzle = create_test_puzzle(&env);
    client.add_puzzle(&puzzle);
    client.join_game(&game1, &player2);

    // Now only one game should be open
    let open_games = client.get_open_games();
    assert_eq!(open_games.len(), 1);
    assert_eq!(open_games.get(0).unwrap(), game2);
}
